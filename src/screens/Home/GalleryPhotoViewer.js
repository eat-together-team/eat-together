// Full-screen profile gallery photo viewer — reached by tapping a photo in
// the Gallery grid. Adapted from EventPhotoViewer.js (see that file's header
// comment for why plain PanResponder + Animated is used instead of Gesture
// Handler/Reanimated): same pinch/pan/dismiss/filmstrip-swipe behavior and
// thumbnail strip, just sourced from a user's profile gallery instead of an
// event's, and with no photo-tagging (not part of this gallery's design).
...
export default function GalleryPhotoViewer({ route, navigation }) {
  const { photos, initialIndex, ownerId } = route.params;
  ...
  const isOwnGallery = ownerId === auth.currentUser?.uid;
  ...
  const confirmDeletePhoto = async () => {
    ...
    try {
      await storage.ref().child(`Gallery/${ownerId}/${photo.imageId}`).delete();
